...
/*
 poziadavky cviciaceho
 */

defined('IN_CMS') or die('No access');

class PractRequirements extends Model
{
// toto je pole, mozno by bodlo uviest strukturu ako je z pohladu
    ...
    //init DB connectu
        ...
        for ($rozl=1;$rozl<=MAX_ROZLOZENI;$rozl++) {
            ...
            for ($poz=1;$poz<=MAX_POZIADAVOK;$poz++) {
                $ch["[layouts][{$rozlID}][requirement][{$poz}][pract_hours]"] = array(
                    "not_empty" => true,
                    "is_int"    => true,
                    "min_value" => 1,
                    "max_value" => 15,
                    "popis"     => "rozsah cvičenia v rozložení {$rozl} pre cvičenie {$poz}"
                );
                $ch["[layouts][{$rozlID}][requirement][{$poz}][pract_paralell]"] = array(
                    "not_empty" => true,
                    "is_int"    => true,
                    "min_value" => 1,
                    "max_value" => 200,
                    "popis"     => "maximálny počet cvičení súčasne v rozložení {$rozl} pre cvičenie {$poz}"
                );
                $ch["[layouts][{$rozlID}][requirement][{$poz}][rooms][1][students_count]"] = array(
                    "not_empty" => true,
                    "is_int"    => true,
                    "min_value" => 1,
                    "max_value" => 10000,
                    "popis"     => "počet študentov v rozložení {$rozl} pre cvičenie {$poz} a typ skupiny 1"
                );
                $ch["[layouts][{$rozlID}][requirement][{$poz}][rooms][2][students_count]"] = array(
                    "not_empty" => true,
                    "is_int"    => true,
                    "min_value" => 1,
                    "max_value" => 10000,
                    "popis"     => "počet študentov v rozložení {$rozl} pre cvičenie {$poz} a typ skupiny 2"
                );
            }
        }
    ...
    {
        return $this->metaRequests->getPreviousMetaID($id_predmet, 2, $metaPoziadavkaID);
    }
    ...
    {
        return $this->metaRequests->getNextMetaID($id_predmet, 2, $metaPoziadavkaID);
    }
    ...
    {
        return array(
        "pract_hours"     => $req["rozsah_hodin"],
        "pract_paralell"  => $req["sucastne"],
        "comment"     	  => $req["ine"],
    ...
    private function __loadSelectedRooms($id_poziadavka_miestnost)
    {
        ...
        $this->dbh->query($sql, $id_poziadavka_miestnost);
        $res = array();
