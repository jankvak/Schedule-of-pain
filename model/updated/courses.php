...
class Courses extends Model {
    ...
    //TODO: tu treba v buducnosti ostrit ze ak je zakazane predavanie vrati -1 (bude potrebna extra query na zistenie co je mozne poziadavky prebrat)
        ...
        $sql =
            "SELECT id FROM course p
 			 WHERE p.id_semester=$1 AND p.name=$2";
    ...
    // extra stlpce berie kvoli gerantovi aby mal vsetky potrebne udaje pre rozhodnutie ci boli zadane poziadavky
        $query =
            "SELECT p.id, p.name, p.abbreviation, p.lecture_hours, p.exercise_hours, vp.id_group
  			 FROM course p
  			 JOIN person_course vp ON vp.id_course=p.id
  			 JOIN course_semester cs ON cs.id_course=p.id
 			 WHERE  vp.id_person=$1 AND cs.id_semester=$2
			 ORDER BY p.name";
        ...
        {
            $result = $this->__compareRoles($role,$predmet["role_type"]);
            //  echo $result;s
            if($result)
            {
                $predmet["prev_id"] = $this->__getPrevPredmetID($predmet["name"], $semester);
    ...
    public function getAll($semesterID)
    {
        $sql =
            "SELECT p.*
			 FROM course p
			 JOIN course_semester cs ON p.id=cs=id_course
    ...
    {
        $sql = "SELECT p.name FROM course p WHERE p.id = $1";
    ...
    // ak je rola vo forme "code" zmenim na jej id
        ...
        $sql =
            "SELECT pc.id FROM person_course pc
             WHERE pc.id_course=$1 AND pc.id_person=$2  AND (pc.role_type<=$3 AND pc.role_type >2)";
