...
//definuje nazvy jednotlivych stavov pripomienok
    ...
    {
        $query =
            "SELECT r.id, r.body, r.timestamp, r.id_person, r.stav, r.report_type ".
    ...
    {
        $query =
            "SELECT r.id, r.body, r.timestamp, r.id_person, r.status, report_type, ".
            Users::vyskladajMeno("pedagog", "name", false).
            "FROM report r, person
    ...
    {
        $sql =
            "SELECT r.id, r.body, r.timestamp, r.status, report_type
            FROM report r
            WHERE r.id_person=$1
    ...
    function getTypes()
    {
        $query = "SELECT UNIQUE role_type FROM report";
        $this->dbh->Query($query);
